    ...
    <div className="px-4 py-5 fade-up flex flex-col gap-4">
      ...
      <div className="flex flex-col gap-2">
        {(tmpl.fields ?? []).map((section: FormSection, idx: number) => {
          ...
          const hasAiPrompt = isAdmin && !!(section as Record<string, unknown>).ai_prompt
          return (
            <div key={section.id} className="card overflow-hidden">
              <div className="w-full flex items-center justify-between p-4">
                ...
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                  {hasAiPrompt && (
                    <button
                      onClick={() => document.getElementById(`prompt-editor-${section.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" })}
                      className="flex items-center gap-1 px-2 py-1 rounded-lg bg-purple-50 border border-purple-200 text-purple-700 text-[10px] font-semibold hover:bg-purple-100 transition-colors">
      ...
      {isAdmin && Object.keys(aiPrompts).length > 0 && (
        <div className="flex flex-col gap-4 mt-4 pt-4 border-t border-gray-100">
          ...
          {Object.entries(aiPrompts).map(([sectionId, prompt]) => {
            ...
            return (
              <div key={sectionId} id={`prompt-editor-${sectionId}`} className="card overflow-hidden">
